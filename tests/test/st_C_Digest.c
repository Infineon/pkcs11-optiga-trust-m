...
void st_c_digest_init_update_final_valid_001() {
    ...
    CK_MECHANISM smech = {.mechanism = CKM_SHA256, .pParameter = NULL, .ulParameterLen = 0};
    ...
    do {
        ...
        if (rv != CKR_OK) {
            printf(
                "C_DigestInit failed with Response Code :%x at line Number : %d\n",
                rv,
                __LINE__
            );
            break;
        ...
        if (rv != CKR_OK) {
            printf(
                "C_DigestUpdate failed with Response Code :%x at line Number : %d\n",
                rv,
                __LINE__
            );
            break;
        ...
        if (rv != CKR_OK) {
            printf(
                "C_DigestFinal failed with Response Code :%x at line Number : %d\n",
                rv,
                __LINE__
            );
            break;
...
void st_c_digest_init_mechanism_invalid_004() {
    ...
    CK_BBOOL true = CK_TRUE;
    CK_BBOOL false = CK_FALSE;
    CK_MECHANISM smech = {.mechanism = CKM_AES_CBC, .pParameter = NULL, .ulParameterLen = 0};
