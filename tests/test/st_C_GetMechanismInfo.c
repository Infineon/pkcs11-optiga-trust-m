...
void st_c_test_get_mechanism_info_valid_001(void) {
    ...
    do {
        ...
        if (rv != CKR_OK) {
            printf(
                "C_GetFunctionList failed with Response Code :%x at line Number : %d\n",
                rv,
                __LINE__
            );
            break;
        ...
        if ((rv == CKR_OK) && (ulCount > 0)) {
            ...
            if (rv != CKR_OK) {
                printf(
                    "C_GetSlotList failed with Response Code :%x at line Number : %d\n",
                    rv,
                    __LINE__
                );
                break;
            ...
            if (rv != CKR_OK) {
                printf(
                    "C_GetMechanismInfo failed with Response Code :%x at line Number : %d\n",
                    rv,
                    __LINE__
                );
                break;
            ...
            rv = function_list
                     ->C_GetMechanismInfo(pSlotList[0], CKM_EC_KEY_PAIR_GEN, &mechanism_info);
            CU_ASSERT_EQUAL(CKR_OK, rv);
            if (rv != CKR_OK) {
                printf(
                    "C_GetMechanismInfo failed with Response Code :%x at line Number : %d\n",
            ...
            memset(&mechanism_info, 0, sizeof(mechanism_info));
            rv = function_list
                     ->C_GetMechanismInfo(pSlotList[0], CKM_RSA_PKCS_KEY_PAIR_GEN, &mechanism_info);
            CU_ASSERT_EQUAL(CKR_OK, rv);
            if (rv != CKR_OK) {
                printf(
                    "C_GetMechanismInfo failed with Response Code :%x at line Number : %d\n",
