...
void st_c_sign_ecc_valid_001() {
	...
	do 
	{
		...
		rv = C_Sign(hSession, sha256_msg_hash, sizeof(sha256_msg_hash), sig,
				&siglen);
		CU_ASSERT_EQUAL( CKR_OK, rv );	
		if (rv != CKR_OK) {
...
void st_c_sign_rsa_1024_valid_different_sign_scheme_003() {
	...
	CK_MECHANISM_TYPE test_mechanism[] = {CKM_SHA256_RSA_PKCS,
										  CKM_SHA384_RSA_PKCS,
										  CKM_SHA512_RSA_PKCS};
	...
	{			
		...
		{		
			...
			/* Call C_Sign for size */
			rv = C_Sign(hSession, message_array[index], message_array_size[index], sig,
					&siglen);
			CU_ASSERT_EQUAL( CKR_OK, rv );	
			if (rv != CKR_OK) {
				printf("C_Sign failed with Response Code :%x at line Number : %d\n",rv, __LINE__);
